...
weights_input_hidden += X.T.dot (d hidden) * learning_rate 
bias_output += np. sumd_output, axis=0, keepdims=True) * learning_rate 
bias_hidden += np. sumd_hidden, axis=0, keepdims=True) * learning_rate
# Testing the neural network
hidden_input = np.dot (X, weights_input_hidden) + bias_hidden 
hidden_output = sigmoid(hidden_input)
output_layer_input = np.dot (hidden_output, weights_hidden_output) 
bias_output output_layer_output=sigmoid (output_layer_input)
# Print the final output
print ("Final Output:")
print(output_layer_output)
# Print the final weights and biases
print("\nFinal Weights and Biases:")
print ("Weights Input to Hidden Layer:") 
